...
// const checkEmail = httpsCallable(functions, 'checkWaitlistEmail');
...
//   submissionCache.set(normalizedEmail, now);
//   setTimeout(() => submissionCache.delete(normalizedEmail), CACHE_DURATION);
  
//   try {
//     // Check if email exists via Cloud Function
//     const result = await checkEmail({ email: normalizedEmail });
//     const exists = (result.data as any).exists;
    
//     // Always track engagement
//     await trackEngagement(normalizedEmail, 'waitlist_signup', {
//       source,
//       timestamp: new Date().toISOString()
//     });
    
//     // Only add to waitlist if doesn't exist
//     if (exists) {
//       return { id: null, referralCode: '', isNew: false };
//     }
